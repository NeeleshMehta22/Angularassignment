...
const getUsers = (req:Request, res:Response) => {
    pool.query('SELECT * ,(SELECT cname FROM schema1.customer as customer where u.id=user_id) FROM userdata as u order by id', (error:any, results:any) => {
      if (error) {
        throw error
      }
  ...
  const updateUser = (req:Request, res:Response) => {
   ...
   const deleteUser = async (req:Request, res:Response) => {
    ...
    await pool.query('DELETE FROM SCHEMA1.customer WHERE SCHEMA1.customer.user_id = $1', [id], (error:any, results:any) => {
